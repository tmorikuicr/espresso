...
.initMapByRandom <- function(obj) {
  n <- nrow(obj@topology)
  genes <- obj@gset
  newmap <- vector('list', length = obj@rep)
  names(newmap) <- paste0('rep.', 1:obj@rep)
  for (s in 1:obj@rep) {
    samples <- obj@ssets[[s]]
    ...
    rep <- matrix(NA, n, length(genes))
    rownames(rep) <- rownames(obj@topology)
    colnames(rep) <- genes
    for (i in 1:n) {
      rep[i, ] <- mi + (ma - mi) * runif(ncol(rep))
    }
    newmap[[s]] <- rep
  }
  obj@map <- newmap
...
.initBMU <- function(obj) {
  n <- nrow(obj@topology)
  genes <- obj@gset
  newbmu <- vector('list', length = obj@rep)
  names(newbmu) <- paste0('rep.', 1:obj@rep)
  for (s in 1:obj@rep) {
    samples <- obj@ssets[[s]]
    exprs <- obj@exprs[match(samples, rownames(obj@exprs)), , drop = FALSE]
    exprs <- exprs[, match(genes, colnames(obj@exprs)), drop = FALSE]
...
.searchBMUByMin <- function(obj) {
  ...
  bmu <- apply(obj@exprs_som, 1, 
               function(i, map = obj@map[[rep]]){
                 x <- matrix(i, nrow = nrow(obj@map[[rep]]), 
                             ncol = ncol(obj@map[[rep]]), byrow = TRUE)
...
.trainGraphSOM <- function(obj) {
  ...
  pb <- progress_bar$new(
    ...
    total = obj@rep*obj@lsteps, clear = FALSE, width = 60)
...
#' @export
#' 
plotConvCurve <- function(obj, rep = NULL, 
  ...
  for (r in rep) {
    ...
    plot(x, y, xlab = 'Learning steps', ylab = 'Difference (#cells)',
         main = paste0('rep.', r), col = col, type = type, lwd = lwd, 
         ylim = c(0, ncell))
  }
}
...
.summarizeScores <- function(obj) {
  summary <- vector("list", length = 6)
  names(summary) <- c("mean_score", "var_score",
                      "mean_acc", "var_acc", 
                      "mean_ari", "var_ari")
  s <- obj@score
  avg <- apply(s, 2, mean)
...
.evalGraphSOM <- function(obj) {
  pb <- progress_bar$new(
    format = "[:bar] :percent eta: :eta",
    total = obj@rep, clear = FALSE, width= 60)
  message("Evaluation")
  score <- matrix(NA, nrow = obj@rep, ncol = 3)
  rownames(score) <- paste('rep.', 1:obj@rep, sep = '')
...
#' @return espresso object
#' 
.genSampleSets <- function(obj, nsamples, rep) {
  domains <- sort(unique(obj@asgmt$domain))
  obj@rep <- rep
  obj@ssets <- vector('list', length = obj@rep)
  names(obj@ssets) <- paste0('set.', 1:obj@rep)
  for (i in 1:obj@rep) {
    selected_samples <- c()
    for (d in domains) {
...
#' @param rep the number of replications (default: 1).
#' @param radius initial value of learning radius .
#'               (default: the maximum distance of input toplology).
...
#' @export
#' 
initGraphSOM <- function(obj, nsamples = NULL, rep = 1, 
                         radius = NULL, lsteps = 100, 
                         stochastic = TRUE, rmin = 0.5, rmax = 1.0, nmin = 1, nmax = NULL,
                         map_method = "sample", bmu_method = "min", seed = 0, coef = 0) {
  
  obj <- .genSampleSets(obj = obj, nsamples = nsamples, rep = as.integer(rep))
...
graphSOM <- function(obj, gset = NULL, seed = 0, verbose = TRUE) {
  ...
  if (length(obj@gset) < obj@nmin || 
    ...
    obj@summary <- list("mean_score" = NA, "var_score" = NA, 
                        "mean_acc" = NA, "var_acc" = NA,
                        "mean_ari" = NA, "var_ari" = NA)
  } else {
    if (verbose == FALSE) {
